...
def add_to_loss_plot_moded(ax, loss_values, label, other_parameters):
    #plot in log scale
    loss_values, idx = modify_loss_values(loss_values)
...
def add_to_solution_plot(ax, X, T, values, label, other_parameters,time):

    ax.set_title("Solution profile")
    ax.plot_surface(X, T, values, cmap=cm.coolwarm , linewidth=0, antialiased=False, label=label)
    ax.set_xlabel("x")
    ax.set_ylabel("t")
    ax.set_zlabel("u")
    ax.set_ylim(-0.1, 1.2)
...
if __name__ == "__main__":
    ...
    for function in functions:
        ...
        axs_solution = axs_solution.flatten()
        ...
        fig_solution.suptitle(f"Solution profile normalized to [0,1] for {function}")
        ...
        for path, ax_loss, ax_loss_raw, ax_solution in zip(paths, axs_loss, axs_loss_raw, axs_solution):
            ...
            data_y = torch.linspace(0,1, len(data)).reshape(-1,1)
            # pp(data)
            # pp(data_y)
            ...
            X, T = np.meshgrid(data, data_y)

            pinn_values = load_values_2d(os.path.join(path, function, "pinn_values.pt")).detach().numpy().reshape(len(data),len(data_y))
